...
public class AddMovieCommand implements ICommand {
   ...
   */
  private Movie getMovieFromJsp(HttpServletRequest request)
    ...
    movie.setReleaseDate(
        Timestamp.valueOf(request.getParameter("releaseDate").replace("T", " ") + ":00"));
    ...
    for (Language language : languageService.findAll()) {
      descriptions.add(
          new MovieDescription(
              request.getParameter(language.getName() + "Title"),
              request.getParameter(language.getName() + "Director"),
              language));
    }
