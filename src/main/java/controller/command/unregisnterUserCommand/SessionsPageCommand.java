...
public class SessionsPageCommand implements ICommand {
  ...
  private static final Logger logger = Logger.getLogger(SessionsPageCommand.class);
  ...
  public String execute(HttpServletRequest request, HttpServletResponse response)
    ...
    if (request.getParameter("groupBy") != null) {
      request.setAttribute(
          "sessions", sessionService.sortBy(request.getParameter("groupBy"), movieId));
    } else {
      request.setAttribute("sessions", sessionService.findByMovieId(movieId));
    }
    request.setAttribute(
        "movie",
        movieService.findByIdAndLanguageId(
            movieId, (Long) request.getSession().getAttribute("dbLanguage")));
