...
public class IndexPageCommand implements ICommand {
  ...
  public String execute(HttpServletRequest request, HttpServletResponse response)
      throws DaoOperationException, ServletException, IOException, TransactionException {
    ...
    if (request.getParameter("movieName") != null) {
      ...
      request.setAttribute(
          "count",
          movieService.getCountMovieWhichHaveSessionInTheFutureByTitleAndLanguageId(
              request.getParameter("movieName"), 1L));
      request.setAttribute(
          "movies",
          movieService.findAllWhichHaveSessionInTheFutureByLanguageAndTitle(
              (Long) request.getSession().getAttribute("dbLanguage"),
              request.getParameter("movieName"),
              start,
              StartPosition.AMOUNT_MOVIE_ON_A_PAGE));
      ...
      request.setAttribute(
          "movies",
          movieService.findAllWhichHaveSessionToday(
              (Long) request.getSession().getAttribute("dbLanguage"),
              start,
              StartPosition.AMOUNT_MOVIE_ON_A_PAGE));
      ...
      request.setAttribute(
          "movies",
          movieService.findAllWhichHaveSessionInTheFutureByLanguage(
              (Long) request.getSession().getAttribute("dbLanguage"),
              start,
              StartPosition.AMOUNT_MOVIE_ON_A_PAGE));
    }
